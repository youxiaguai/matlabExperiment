...
f = @(x) (-x.^2+5);   % 函数表达式
ezplot(f, [0, 31])             % 画出函数图像

N = 50;                          % 种群上限 一般设置为20-200
ger = 100;                       % 迭代次数 一般为100-300
L = 5;                           % 基因长度
pc = 0.8;                        % 交叉概率 一般为0.3-0.9
pm = 0.1;                        % 变异概率 一般为0.001-0.1
dco = [10000; 1000; 100; 10 ;1]; % 解码器
dna = randi([0, 9], [N, L]); % 基因
hold on
x = dna * dco / 99999 ;          % 对初始种群解码
